...
try:
    ...
    lower_green = np.array([80/360*179, 0, 30])
    ...
    #print(hierarchy)
    total = 0
    cv2.imwrite("colour_original.png",color_image)
    #print(hierarchy)
    ...
    saveFileName = datenow.strftime("%Y%m%d_%H%M%S.png")
    saveFileName = os.path.join(savedir, saveFileName)
    saveFileName2 = datenow.strftime("%Y%m%d_%H%M%S_mask.png")
    saveFileName2 = os.path.join(savedir, saveFileName2)

    cv2.imwrite(saveFileName, color_image)
    cv2.imwrite(saveFileName2, mask_depth)
